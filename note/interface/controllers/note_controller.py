...
class UpdateNoteBody(BaseModel):
    ...
    content: str | None = Field(default=None, min_lenth=1)
...
def create_note(
    ...
    body: CreateNoteBody,
    note_service: NoteService = Depends(Provide["note_service"]),
):
    ...
    response = asdict(note) # asdict는 dataclass 객체를 dict로 변환하는데 사용되는 모듈
...
def update_note(
    ...
    current_user: Annotated[CurrentUser, Depends(get_current_user)],
    body: UpdateNoteBody,
    note_servcie: NoteService = Depends(Provide["note_service"]),
):
    note = note_servcie.update_note(
        user_id= current_user.id,
        id=id, 
        title=body.title, 
        content=body.content, 
...
def get_notes_by_tags(
    tag_name: str, 
    current_user: Annotated[CurrentUser, Depends(get_current_user)], # 기본값 없는 인자가 기본값 있는 인자 뒤에 올 수 없어 인자 순서 조정. 
    note_servcie: NoteService = Depends(Provide["note_service"]),
    page: int=1,
    items_per_page: int= 10,    
):
    total_count, notes = note_servcie.get_notes_by_tag(
        user_id =current_user.id, 
        tag_name=tag_name, 
        page=page, 
        items_per_page=items_per_page,
